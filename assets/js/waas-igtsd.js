(function () {
  ...
  const initHeroSlider = () => {
    ...
    // Inject autoplay progress bar
    let progress = hero.querySelector('.hero-premium-progress');
    if (!progress) {
      progress = document.createElement('div');
      progress.className = 'hero-premium-progress';
  ...
  const initKpiCounters = () => {
    const grid = document.querySelector('.feature-bar .feature-grid');
    if (!grid) return;
    const items = Array.from(grid.querySelectorAll('.kpi-value'));
    ...
    const animateCounter = (el) => {
      ...
      const cleaned = finalText.replace(/\s/g, '');
      const matches = cleaned.match(/\d+/g) || [];
      if (!matches.length) return;
      const target = Math.max.apply(null, matches.map(Number));
      if (!Number.isFinite(target) || target <= 0) return;

      // Detect non-numeric trailing suffix (e.g. " km")
      const suffixMatch = finalText.match(/[^\d\s]+\s*$/);
      ...
      const duration = Math.min(1600 + Math.log10(target) * 220, 2400);
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting && !grid.dataset.revealed) {
          grid.dataset.revealed = '1';
          grid.classList.add('is-revealed');
          // Slight delay so counters start near the end of the card fade-in
          setTimeout(() => items.forEach(animateCounter), 350);
          observer.disconnect();
        }
      });
    }, { threshold: 0.3 });
